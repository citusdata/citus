			 ...
			 PQExpBuffer query_buf)
{
	...
	/* \stage */
	else if (pg_strcasecmp(cmd, "stage") == 0)
	{
		/*
		 * The stage command augments psql's copy command with data loading
		 * functionality for sharded databases. Specifically, the client first
		 * contacts the master node and fetches shard and node metadata. Then,
		 * the client coordinates the loading of data to remote nodes.
		 */
		instr_time startTime;
		char *stageCommand = NULL;
		...
		/* read the rest of line; do not strip away quotes or semicolons */
		stageCommand = psql_scan_slash_option(scan_state, 
											  OT_WHOLE_LINE, NULL, false);

		success = DoStageData(stageCommand);
		...
		if (pset.timing && success)
		{
			...
			INSTR_TIME_SET_ZERO(elapsedTime);

			INSTR_TIME_ACCUM_DIFF(elapsedTime, endTime, startTime);
			printf(_("Time: %.3f ms\n"), INSTR_TIME_GET_MILLISEC(elapsedTime));
		}
