...
def getTableData():
    ...
    for table in tables:
        # generate base rows
        if table.useRandom:
            dataGenerationSql += _genDataRandom(table.name, fromVal, toVal, table.rowCount)
        else:
...
def _genData(tableName, startVal, rowCount):
    ...
    dataGenerationSql = ''
    dataGenerationSql += 'INSERT INTO ' + tableName
    dataGenerationSql += ' SELECT i FROM generate_series(' + str(startVal) + ',' + str(startVal+rowCount) + ') i;'
    return dataGenerationSql
...
def _genDifferentData(tableName, startVal, tableIdx):
    ...
    dataGenerationSql = ''
    dataGenerationSql += 'INSERT INTO ' + tableName
    dataGenerationSql += ' SELECT i FROM generate_series(' + str(startVal) + ',' + str(endVal) + ') i;'
    return dataGenerationSql
