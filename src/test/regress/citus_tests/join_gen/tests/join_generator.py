...
def run_test(config):
    ...
    process = subprocess.Popen(testRunCommand.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = process.communicate()

    # stop cluster
    common.stop_databases(
        config.bindir, config.datadir,
        config.node_name_to_ports, config.name
    )
...
if __name__ == '__main__':
    citusBaseClusterConfig = cfg.CitusBaseClusterConfig(docopt(__doc__, version="join_gen_test"))
