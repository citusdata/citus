...
-- Show that we don't try to do parallel scan on columnar table during index build.
-- Otherwise, below CREATE INDEX would error out at columnar_parallelscan_estimate.
...
-- We temporarily set max_parallel_maintenance_workers to 0 to prevent index-only
-- scan's on columnar tables.
-- Make sure that we rollback this change after finishing the DDL, so this should
-- print "3".
