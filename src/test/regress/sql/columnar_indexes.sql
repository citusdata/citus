...
-- REINDEX). The table's parallel_workers setting makes these builds parallel
-- eligible unless the columnar planner/utility guards keep them serial.
SET max_parallel_maintenance_workers = 4;
CREATE TABLE guc_restore_test(a int) USING columnar WITH (parallel_workers = 4);
INSERT INTO guc_restore_test SELECT i FROM generate_series(1,10000) i;
CREATE INDEX ON guc_restore_test (a);
SHOW max_parallel_maintenance_workers;
SELECT reltuples = 10000
FROM pg_class
WHERE oid = 'guc_restore_test_a_idx'::regclass;
