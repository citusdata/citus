...
EXPLAIN (COSTS FALSE, ANALYZE FALSE, TIMING FALSE, SUMMARY FALSE, VERBOSE TRUE, BUFFERS OFF)
	SELECT l_quantity, count(*) count_quantity FROM lineitem
	GROUP BY l_quantity ORDER BY count_quantity, l_quantity;
...
EXPLAIN (COSTS FALSE, BUFFERS OFF) SELECT avg(l_linenumber) FROM lineitem_clone;
DROP TABLE lineitem_clone;

-- ensure distributed plans don't break
EXPLAIN (COSTS FALSE, BUFFERS OFF) SELECT avg(l_linenumber) FROM lineitem;

-- ensure EXPLAIN EXECUTE doesn't crash
PREPARE task_tracker_query AS
	SELECT avg(l_linenumber) FROM lineitem WHERE l_orderkey > 9030;
EXPLAIN (COSTS FALSE, BUFFERS OFF) EXECUTE task_tracker_query;
...
PREPARE real_time_executor_query AS
	SELECT avg(l_linenumber) FROM lineitem WHERE l_orderkey > 9030;
EXPLAIN (COSTS FALSE, BUFFERS OFF) EXECUTE real_time_executor_query;
...
BEGIN;
EXPLAIN (COSTS OFF, BUFFERS OFF) EXECUTE multi_shard_query_param(5);
ROLLBACK;
BEGIN;
...
EXPLAIN (COSTS OFF, VERBOSE true, BUFFERS OFF)
...
\set default_explain_flags '(ANALYZE off, COSTS off, TIMING off, SUMMARY off, BUFFERS off)'
...
EXPLAIN (COSTS OFF, BUFFERS OFF) EXECUTE delete_query('x', 'y');
...
EXPLAIN (COSTS false, BUFFERS OFF) EXECUTE q1('(1)');
...
EXPLAIN (COSTS false, BUFFERS OFF) EXECUTE q2('(1)');
