...
-- vericfy that the UDFs will fail on NULL input
...
-- Reset the stats for all before the next test.
--
...
SELECT query_execution_single_shard is NULL, query_execution_multi_shard is NULL FROM citus_stat_counters;

-- Even further, for the databases that don't have Citus extension installed,
-- we should get NULL for other stats too.
--
-- For the databases that have Citus extension installed, we might or might not
-- get NULL for connection related stats, depending on whether the Citus maintenance
-- daemon has done any work so far, so we don't check them.
SELECT connection_establishment_succeeded is NULL,
       connection_establishment_failed is NULL,
       connection_reused is NULL
FROM (
...
-- no single shard queries yet, so it's set to NULL
SELECT query_execution_single_shard is NULL FROM (SELECT (citus_stat_counters(oid)).* FROM pg_database WHERE datname = current_database()) q;
...
-- verify that we reset stats for all databases by default (because we pass 0)
SELECT citus_stat_counters_reset();
SELECT query_execution_single_shard is NULL FROM (SELECT (citus_stat_counters(oid)).* FROM pg_database WHERE datname = current_database()) q;
...
-- For the last two queries, we forced opening 32*2=64 connections to the
-- workers, so connection_establishment_succeeded must "at least"(*) be 64.
-- (*) previous queries opened some number of connections too
SELECT query_execution_single_shard = 4, query_execution_multi_shard = 5, connection_establishment_succeeded >= 64
