...
BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED;
	...
	SET application_name to 'citus';
	\set VERBOSITY terse
	WITH distributed_object_data(typetext, objnames, objargs, distargumentindex, colocationid, force_pushdown)
		AS (VALUES ('non_existing_type', ARRAY['non_existing_user']::text[], ARRAY[]::text[], -1, 0, false))
	SELECT citus_internal_add_object_metadata(typetext, objnames, objargs, distargumentindex, colocationid, force_pushdown) FROM distributed_object_data;
ROLLBACK;
