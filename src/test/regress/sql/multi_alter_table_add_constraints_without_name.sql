...
-- Check "ADD EXCLUDE" with an expression, a schema-qualified operator and a WHERE clause
ALTER TABLE AT_AddConstNoName.products ADD EXCLUDE USING btree (product_no WITH OPERATOR(pg_catalog.=), (lower(name)) WITH =) WHERE (price > 0);
\c - - :public_worker_1_host :worker_1_port
SELECT conname, pg_get_constraintdef(oid)
  FROM pg_catalog.pg_constraint
 WHERE conrelid = 'AT_AddConstNoName.products_5410000'::regclass
   AND contype = 'x';

\c - - :master_host :master_port
ALTER TABLE AT_AddConstNoName.products DROP CONSTRAINT products_product_no_expr_excl;
