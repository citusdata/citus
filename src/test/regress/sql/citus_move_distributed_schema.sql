CREATE SCHEMA citus_move_distributed_schema;
SET search_path TO citus_move_distributed_schema;
...
SELECT citus_move_distributed_schema(schema_id=>null, target_node_name=>null, target_node_port=>null, shard_transfer_mode=>null);
SELECT citus_move_distributed_schema(schema_id=>null, target_node_id=>null, shard_transfer_mode=>null);
SELECT citus_move_distributed_schema_with_nodeid(schema_id=>null, target_node_id=>null, shard_transfer_mode=>null);
...
-- test invalid schema
SELECT citus_move_distributed_schema('no_such_schema', 'dummy_node_name', 1234);
SELECT citus_move_distributed_schema_with_nodeid('no_such_schema', 1234);

-- test regular schema
SELECT citus_move_distributed_schema('citus_move_distributed_schema', 'dummy_node_name', 1234);
SELECT citus_move_distributed_schema_with_nodeid('citus_move_distributed_schema', 1234);
...
SELECT citus_move_distributed_schema('s1', nodename, nodeport) FROM pg_dist_node
...
SELECT citus_move_distributed_schema('s1', nodename, nodeport, 'block_writes')
FROM citus_shards
JOIN pg_dist_node USING (nodename, nodeport)
WHERE noderole = 'primary' AND table_name = 's1.t1'::regclass;

SELECT citus_move_distributed_schema('s1', nodeid, 'block_writes')
FROM citus_shards
JOIN pg_dist_node USING (nodename, nodeport)
WHERE noderole = 'primary' AND table_name = 's1.t1'::regclass;
...
SELECT citus_move_distributed_schema('s1', :s1_new_nodename, :s1_new_nodeport, 'block_writes');
...
SELECT citus_move_distributed_schema_with_nodeid('s1', :s1_new_nodeid, 'block_writes');

SELECT (:s1_new_nodename, :s1_new_nodeport) = ALL(SELECT nodename, nodeport FROM citus_shards JOIN pg_dist_node USING (nodename, nodeport) WHERE noderole = 'primary' AND starts_with(table_name::text, 's1'::text));

-- move the schema to the coordinator

SELECT citus_move_distributed_schema('s1', 'localhost', :master_port, 'block_writes');
...
SELECT citus_move_distributed_schema('s2', 'dummy_node', 1234);

-- assign all tables to regularuser
RESET ROLE;
SELECT result FROM run_command_on_all_nodes($$ REASSIGN OWNED BY tenantuser TO regularuser; $$);

GRANT USAGE ON SCHEMA citus_move_distributed_schema TO regularuser;
...
SELECT citus_move_distributed_schema('s2', :s2_new_nodename, :s2_new_nodeport, 'force_logical');
...
REVOKE USAGE ON SCHEMA citus_move_distributed_schema FROM regularuser;
DROP ROLE regularuser, tenantuser;

RESET citus.enable_schema_based_sharding;

SET client_min_messages TO WARNING;
DROP SCHEMA citus_move_distributed_schema, s1 CASCADE;
