...
below steps. First make sure these two thing are the case:
...
git pull # Make sure your local enterprise-master is up to data
...
2. Wait until tests on `enterprise/$PR_BRANCH`.
...
6. You can now merge the PR on community. Be sure NOT to use "squash and merge",
   but instead use the regular "merge commit" mode.
7. You can now merge the PR on enterprise. Be sure NOT to use "squash and merge",
   but instead use the regular "merge commit" mode.
