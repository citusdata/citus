...
RegisterCitusConfigVariables(void)
{
	...
	DefineCustomIntVariable(
		...
		gettext_noop("Sets how often the cached cluster minimum Citus version is "
					 "invalidated so it gets recomputed."),
		gettext_noop("The maintenance daemon periodically invalidates the "
					 "shared-memory cache used by citus_minimum_cluster_version() so "
					 "that the next call recomputes it. This bounds how stale the "
					 "cached value can be, for example after a node is upgraded in "
					 "place. Use -1 to disable."),
