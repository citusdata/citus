...
- [DDL](#ddl)
  ...
  - [Sequence handling when initiating DDL from a worker](#sequence-handling-when-initiating-ddl-from-a-worker)
...
Reference table replication can also be triggered implicitly when a distributed-schema table is created from a worker and one or more reference tables are not yet replicated to all nodes. In that case, after acquiring the colocation locks discussed in [Citus Advisory Locks 4: Colocation locks](#citus-advisory-locks-4-colocation-locks) and the table-level `ShareLock` on `pg_dist_node` described under [Lock Types](#lock-types) on the coordinator, the worker connects to the coordinator and initiates the shard-copy operations from there.
...
Group id 0 is treated almost everywhere in the code as "the coordinator". Several UDFs that can now be invoked from a worker, such as `worker_apply_sequence_command()` and `citus_internal.adjust_identity_column_seq_settings()`, inspect the local group id to decide whether they are running on the coordinator or on a worker. For coordinator and metadata-synced workers, the local group id is always assigned correctly. However, although not recommended, a user can call `stop_metadata_sync_to_node()` on a worker to stop metadata syncing to it. Previously, this also reset that worker's local group id back to 0, which would make the same worker indistinguishable from the coordinator for the UDFs above. Citus therefore defers resetting the local group id until the worker is actually removed from the metadata. Ideally we would not send sequence create/alter commands to workers without metadata at all, since sequences are only used when querying shell tables and never in shard-level queries or DMLs. Reworking that could require a lot more work, and deferring the group-id reset until removal is correct on its own.
...
**Worker nodes**: Nodes with groupid > 0, which can perform reads and writes. Most administrative operations still have to be initiated on the coordinator, but a few DDL paths (notably create / alter of distributed-schema tables) can now be initiated from a worker. Even in those paths the coordinator remains the source of truth for metadata sequences that have never been synced to workers (next colocation id, shard id, shard placement id) and for the colocation locks discussed in [Citus Advisory Locks 4: Colocation locks](#citus-advisory-locks-4-colocation-locks) and the table-level `ShareLock` on `pg_dist_node` described under [Lock Types](#lock-types).
...
A few DDL paths, currently create and alter of distributed-schema tables, can also be initiated from a worker. In those paths we use `EnsurePropagationToCoordinator()` instead of `EnsureCoordinator()`, so that the operation is allowed to start on a worker but the coordinator is required to be in the metadata. When sending the command to the rest of the cluster, we use `SendCommandToRemoteNodesWithMetadata()` instead of `SendCommandToWorkersWithMetadata()`, which excludes the local node from the remote send. Also, since metadata sequences have never been synced to workers, i.e., the next colocation id, shard id, and shard placement id, such sequences are fetched from the coordinator since the coordinator's copy is the source of truth. Then, the rest of the operation is performed on the local node. The same paths also have to acquire the colocation locks discussed in [Citus Advisory Locks 4: Colocation locks](#citus-advisory-locks-4-colocation-locks) and the table-level `ShareLock` on `pg_dist_node` described under [Lock Types](#lock-types) remotely on the coordinator and may trigger implicit reference table replication (see the [Reference tables](#citus-concepts) definition). Sequence-related concerns specific to these worker-initiated paths are described in [Sequence handling when initiating DDL from a worker](#sequence-handling-when-initiating-ddl-from-a-worker).
...
## Sequence handling when initiating DDL from a worker

In Postgres, a column can be associated with a sequence in three ways:
* an explicit default that uses `nextval('sequence_name')`,
* one of the serial pseudo-types (`smallserial`, `serial`, `bigserial`),
* an identity column (`GENERATED ... AS IDENTITY`).

Such a column can show up on a Citus table in three scenarios:
...
When the table is created or altered on the coordinator, Citus enforces the following:

* The coordinator is always allowed to use the full sequence range.
* If the sequence is bigint-based, the allowed sequence range is split across workers based on node group ids. Otherwise, workers are effectively prevented from using the sequence by setting the current value to the maximum value. See `AlterSequenceMinMax()`.
* For columns that have an explicit `nextval()` default, the worker-side default is rewritten to `worker_nextval()` so that invoking it on a worker produces a clear error. For serial and identity columns the default expression cannot be rewritten, but the same range restrictions still apply.
* Since the sequence already lived on the coordinator before distribution, its `last_value` and `is_called` state are implicitly preserved on the coordinator afterwards, so the coordinator continues generating values from where it left off.

We want the same end state when a distributed-schema table is created or altered from a worker. To get there:
...
* `AlterSequenceMinMax()` is executed on the local worker in addition to the remote workers. That way, the local worker either uses the appropriate sub-range (for bigint-based sequences) or is itself prevented from consuming the sequence (by capping the current value at the maximum), exactly like every other worker.
* The sequence's `last_value` and `is_called` state are synced from the worker back to the coordinator's copy of the sequence after such a create / alter operation completes. The coordinator can then continue using the full range starting from the value the worker reached while the table was a regular Postgres table, matching what would happen if the table had been created on the coordinator.
...
+ When regular commands like `INSERT`/`UPDATE`/`DELETE`/`SELECT` is executed, Citus again relies on Postgres to acquire the table-level locks. PostgreSQL acquires the table-level locks during the parsing of the statements, which makes life simple for Citus, as parsing happens even before any Citus logic kicks in. If the command doesn’t require parsing, such as prepared statements, then Postgres still acquires the same locks before using the cached plan. So, from Citus’ perspective, there mostly is nothing to do for acquiring the table-level locks.
 + + There is only a one exception to this rule, Citus' _local plan caching_. When Citus caches the queries by itself, Citus acquires the relevant table-level locks. See `ExecuteLocalTaskListExtended()` as the relevant C function.
...
One another use case for table-level locks on Citus is the table-level locks acquired on the Citus metadata tables. Citus uses table-level locks on the metadata tables to define the concurrency behavior of certain operations. For example, while creating a new table or moving shards, it is common to acquire `ShareLock` on `pg_dist_node` table, and `citus_add_node` function to acquire `ExclusiveLock` on the same metadata table. The latter signals the rest of the backends that the node metadata is about to change, so it is not allowed to rely on the current state of `pg_dist_node` (or vice-versa `citus_add_node` should wait until rebalance finishes). When a table-creation path that can be initiated from a worker (today, creating a distributed-schema table) runs on a worker, the same `ShareLock` on `pg_dist_node` is acquired remotely on the coordinator as well, so it still conflicts with `citus_add_node` exactly like the coordinator-initiated path does.
...
For DDL paths that can be initiated from a worker, currently the create / alter of distributed-schema tables, Citus has to acquire these colocation locks remotely on the coordinator rather than on the local worker, so that they still serialize against coordinator-only operations like shard moves. The colocation locks acquired remotely on the coordinator in that case are:

* the advisory colocation id lock,
* the advisory placement colocation lock (`AcquirePlacementColocationLock()` above).

These are also what makes implicit reference table replication safe when triggered from a worker (see the [Reference tables](#citus-concepts) definition). Note that the table-level `ShareLock` on `pg_dist_node` that these paths additionally take remotely on the coordinator is described separately under [Lock Types](#lock-types).

Within the scope of these locks, code paths that can never be initiated from a worker do not check the local node role at all and they just acquire the lock locally as before. The "should we acquire this remotely on the coordinator?" branch only exists in the worker-initiable paths.
...
Historically, all DDL had to go through the coordinator. That is no longer strictly true: create and alter of distributed-schema tables can now be initiated from any node. The mechanics behind this (like calling `EnsurePropagationToCoordinator()` instead of `EnsureCoordinator()`, sending remote commands via `SendCommandToRemoteNodesWithMetadata()`, fetching colocation / shard / placement id sequences from the coordinator, acquiring the colocation locks discussed in [Citus Advisory Locks 4: Colocation locks](#citus-advisory-locks-4-colocation-locks) and the table-level `ShareLock` on `pg_dist_node` described under [Lock Types](#lock-types)) on the coordinator, and the sequence handling in [Sequence handling when initiating DDL from a worker](#sequence-handling-when-initiating-ddl-from-a-worker) are described in the relevant sections.

The set of DDLs and UDFs that can be initiated from a worker is still limited.

Beyond that, we still need a mechanism to avoid "avoidable distributed deadlocks" that can occur when DDL is initiated from arbitrary nodes; see https://github.com/citusdata/citus/issues/1083 for one possible approach.
