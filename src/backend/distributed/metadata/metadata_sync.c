...
static void FetchSequenceState(Oid sequenceId, uint64 *lastValue, bool *isCalled);
 ...
 */
static void
FetchSequenceState(Oid sequenceId, uint64 *lastValue, bool *isCalled)
{
	char *qualifiedSequenceName = generate_qualified_relation_name(sequenceId);
	...
	PG_TRY();
	{
		...
		if (lastValueInt64 < 0)
		{
			elog(ERROR, "last_value for sequence %s is negative (%lld)",
