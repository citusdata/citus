...
/*
 * AdjustDependentSeqRangesOnLocalWorker takes a table oid, finds all sequences
 ...
 *
 * The function assumes that it's run on a worker node.
 */
void
AdjustDependentSeqRangesOnLocalWorker(Oid relationId)
{
...
/*
 * AdjustIdentityColumnSeqRangesOnLocalWorker takes a table oid, finds all identity
 * columns on the table, and runs AlterSequenceMinMax() for each underlying sequence.
 *
 * The function assumes that it's run on a worker node.
 */
void
AdjustIdentityColumnSeqRangesOnLocalWorker(Oid relationId)
{
	Assert(!IsCoordinator());
...
/*
 * AdjustNextValColumnDefaultsOnLocalWorker takes a table oid,
 ...
 */
void
AdjustNextValColumnDefaultsOnLocalWorker(Oid relationId)
{
	Assert(!IsCoordinator());
	...
	TupleDesc copiedTupleDescriptor = CreateTupleDescCopyConstr(tupleDescriptor);
	relation_close(tableRelation, AccessShareLock);

	TupleConstr *tupleConstraints = copiedTupleDescriptor->constr;
	AttrNumber defaultValueIndex = 0;
	for (int attributeIndex = 0; attributeIndex < copiedTupleDescriptor->natts;
		 attributeIndex++)
	{
		Form_pg_attribute attributeForm = TupleDescAttr(copiedTupleDescriptor,
														attributeIndex);
...
worker_apply_sequence_command(PG_FUNCTION_ARGS)
{
	 ...
	 * such calls.
	 */
	bool newParamsProvided = false;
	int64 lastValue = 0;
	bool isCalled = false;
	if (PG_NARGS() == 4)
	{
		newParamsProvided = true;
		lastValue = PG_GETARG_INT64(2);
		isCalled = PG_GETARG_BOOL(3);
	}
	...
	if (IsCoordinator())
	{
		 ...
		 */
		if (!newParamsProvided)
		{
			ereport(ERROR,
					(errmsg("last_value and is_called must be provided when adjusting "
							"sequence setting on coordinator")));
		}
