 ...
 *   a single-valued filter on the partition column.
 *
...
static DistributedSubPlan * CreateDistributedSubPlan(Query *subPlanQuery,
													 RecursivePlanningContext *context);
...
RecursivelyPlanCTEs(Query *query, RecursivePlanningContext *planningContext)
{
	...
	foreach(cteCell, query->cteList)
	{
		...
		uint32 subPlanId = list_length(planningContext->subPlanList);
		...
		{
			return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
								 "cannot handle CTEs when the router executor "
								 "is disabled",
								 NULL, NULL);
		}
		...
		if (log_min_messages >= DEBUG1)
		{
			...
			elog(DEBUG1, "recursively planning CTE %s: %s", cteName, subPlanString->data);
		}
		...
		subPlan = CreateDistributedSubPlan(subPlanQuery, planningContext);
...
CreateDistributedSubPlan(Query *subPlanQuery, RecursivePlanningContext *context)
{
	DistributedSubPlan *subPlan = NULL;
	int cursorOptions = 0;
	...
	{
		/*
		 * Make sure we go through distributed planning if there are read_records_file
		 * calls, even if there are no distributed tables in the query anymore.
		 */
...
BuildSubPlanResultQuery(Query *subquery, uint64 planId, uint32 subPlanId)
{
	...
	/* build the copy_format parameter for the call to read_intermediate_result */
	...
	resultFormatConst->constvalue = copyFormatId;
