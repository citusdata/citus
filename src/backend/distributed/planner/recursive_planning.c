...
static void RecursivelyPlanNonColocatedSubqueriesInWhere(Query *query,
														 ...
														 RecursivePlanningContext *
														 recursivePlanningContext);
static void RecursivelyPlanRecurringLeftJoins(Query *query, Node *node,
											  RecursivePlanningContext *context);
static bool IsJoinNodeRecurring(Node *joinNode, Query *query);
static void RecursivelyPlanNonRecurringJoinNode(Node *distributedNode, Query *query,
												RecursivePlanningContext *context);
...
RecursivelyPlanSubqueriesAndCTEs(Query *query, RecursivePlanningContext *context)
{
	 ...
	 * types.
	 */
	RecursivelyPlanRecurringLeftJoins(query, (Node *) query->jointree, context);
...
/*
 * RecursivelyPlanRecurringLeftJoins descends into a join tree and recursively
 * plans all non-recurring (i.e., distributed) rels that that participate in an
 * outer join expression, such as <distributed> in "<recurring>" LEFT JOIN <distributed>".
 */
static void
RecursivelyPlanRecurringLeftJoins(Query *query, Node *node,
								  RecursivePlanningContext *recursivePlanningContext)
{
	if (node == NULL)
	{
	...
	else if (IsA(node, FromExpr))
	{
		...
		foreach(fromExprCell, fromExpr->fromlist)
		{
			...
			RecursivelyPlanRecurringLeftJoins(query, fromElement,
											  recursivePlanningContext);
		}
	...
	else if (IsA(node, JoinExpr))
	{
		...
		RecursivelyPlanRecurringLeftJoins(query, joinExpr->larg,
										  recursivePlanningContext);
		RecursivelyPlanRecurringLeftJoins(query, joinExpr->rarg,
										  recursivePlanningContext);
		switch (joinExpr->jointype)
		{
			case JOIN_LEFT:
			{
				...
				{
					RecursivelyPlanNonRecurringJoinNode(rightNode, query,
														recursivePlanningContext);
					return;
				}
			...
			case JOIN_RIGHT:
			{
				...
				if (!leftNodeRecurs && rightNodeRecurs)
				{
					RecursivelyPlanNonRecurringJoinNode(leftNode, query,
														recursivePlanningContext);
					return;
				}
 ...
 * node that is known to be inner side of an outer join.
 */
static void
RecursivelyPlanNonRecurringJoinNode(Node *distributedNode, Query *query,
									RecursivePlanningContext *recursivePlanningContext)
{
	...
	{
		/*
		 * We don't expect RecursivelyPlanRecurringLeftJoins to try recursively
		 * plan such an RTE.
		 */
