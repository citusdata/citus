...
PrepareRenameIdentitiesCommands(Oid sourceRelationId, Oid targetRelationId, List **outCoordinatorCommands, List **outWorkerCommands)
{
	...
	for (int attributeIndex = 0; attributeIndex < targetTupleDescriptor->natts;
		 attributeIndex++)
	{
		Form_pg_attribute attributeForm = TupleDescAttr(targetTupleDescriptor, attributeIndex);

		if (attributeForm->attidentity)
		{
			char *columnName = NameStr(attributeForm->attname);

			Oid targetSequenceOid = getIdentitySequence(targetRelationId, attributeForm->attnum,
											 missing_sequence_ok);
			char *targetSequenceName = generate_relation_name(targetSequenceOid, NIL);

			Oid sourceSequenceOid = getIdentitySequence(sourceRelationId, attributeForm->attnum,
											 missing_sequence_ok);
			char *sourceSequenceName = generate_relation_name(sourceSequenceOid, NIL);

			// to rename sequence on the coordinator
			*outCoordinatorCommands = lappend(*outCoordinatorCommands, psprintf("SET citus.enable_ddl_propagation TO OFF; ALTER SEQUENCE %s RENAME TO %s; RESET citus.enable_ddl_propagation;",
							 quote_identifier(targetSequenceName),
							 quote_identifier(sourceSequenceName)));

			// update workers to use existing sequence and drop the new one generated by PG
			bool missingTableOk = true;
			*outWorkerCommands = lappend(*outWorkerCommands, GetAlterColumnWithNextvalDefaultCmd(sourceSequenceOid, sourceRelationId, columnName, missingTableOk));


			// drop the sequence generated by identity column
			*outWorkerCommands = lappend(*outWorkerCommands, psprintf("DROP SEQUENCE IF EXISTS %s",
										quote_identifier(targetSequenceName)));
		}
	}
}
			 ...
			 bool suppressNoticeMessages, char conversionType)
{
	...
	*/
	...
	PrepareRenameIdentitiesCommands(sourceId, targetId, &outCoordinatorCommandsToRenameIdentites, &outWorkerCommandsToRenameIdentites);
