...
static void CopyToExistingShards(CopyStmt *copyStatement, char *completionTag, Oid
								 relationId);
...
CitusBulkloadCopy(CopyStmt *copyStatement, char *completionTag)
{
	...
	PG_TRY();
	{
		...
		{
			BulkloadCopyServer(copyStatement, completionTag, masterNodeAddress,
							   relationId);
		}
	}
	PG_CATCH();
				   ...
				   NodeAddress *masterNodeAddress, Oid relationId)
{
	...
	foreach(workerCell, workerNodeList)
	{
		...
		else
		{
			...
			if (querySent == 0)
			{
				elog(WARNING, "send bulkload copy to %s:%d failed: %s", nodeName,
					 nodePort,
					 PQerrorMessage(conn));
			}
			else
			{
				...
				else
				{
					...
					{
						elog(WARNING, "epoll_ctl add socket of %s:%d failed", nodeName,
							 nodePort);
					}
					else
					{
					...
					else
					{
						if (!PQisBusy(conn))
						{
							...
							else
							{
								...
								{
									elog(WARNING, "%s:%s error:%s", PQhost(conn), PQport(
											 conn),
...
DeparseCopyStatementOptions(List *options)
{
	...
	foreach(option, options)
	{
		...
		{
			appendStringInfoString(optionsStr, ", ");
		}
		...
		{
			appendStringInfo(optionsStr, "oids %s", defGetBoolean(defel) ? "true" :
							 "false");
		}
		else if (strcmp(defel->defname, "freeze") == 0)
		{
			appendStringInfo(optionsStr, "freeze %s", defGetBoolean(defel) ? "true" :
							 "false");
		}
		...
		{
			appendStringInfo(optionsStr, "header %s", defGetBoolean(defel) ? "true" :
							 "false");
		}
	...
	if (pipe)
	{
		...
		while (!copyDone)
		{
			...
			switch (mtype)
			{
				...
				default:
				{
					elog(ERROR, "unexpected message type 0x%02X during COPY from stdin",
						 mtype);
				}
				break;
			}
...
MasterDistributedTableCacheEntry(RangeVar *relation)
{
	...
	if (PQresultStatus(queryResult) == PGRES_TUPLES_OK)
	{
		...
		partitionKeyString = PQgetvalue(queryResult, 0, Anum_pg_dist_partition_partkey -
										1);
		colocationid = PQgetvalue(queryResult, 0, Anum_pg_dist_partition_colocationid -
								  1);
