 ...
 *      transaction, so today it's not possible to release a transactional advisory lock
 *      in a different command. This is also the reason why Postgres provides
															...
															int *nDependTasks)
{
	...
	*nDependTasks = 0;
	...
	const char *transferModeString =
		...
		transferMode == TRANSFER_MODE_FORCE_LOGICAL_AUTO_IDENTITY ?
