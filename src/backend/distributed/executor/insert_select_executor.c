...
static bool AnyConnectionUsedForMultiplePlacements(Oid relationId);
...
CoordinatorInsertSelectExecScan(CustomScanState *node)
{
	...
	if (!scanState->finishedRemoteScan)
	{
		...
		if (insertSelectQuery->onConflict || insertSelectQuery->returningList != NIL ||
			AnyConnectionUsedForMultiplePlacements(targetRelationId))
		{
			/*
			...
			char *resultIdPrefix = distributedPlan->intermediateResultIdPrefix;
			...
			if (prunedTaskList != NIL)
			{
				...
				workerJob->taskList = prunedTaskList;
...
/*
 * AnyConnectionUsedForMultiplePlacements determines whether any connection
 * was used for multiple shard placements (or co-located groups of shard
 * placements) of the given relation.
 */
static bool
AnyConnectionUsedForMultiplePlacements(Oid targetRelationId)
{
	ListCell *shardCell = NULL;
	List *shardList = LoadShardList(targetRelationId);
	...
	foreach(shardCell, shardList)
	{
		...
		List *placementList = FinalizedShardPlacementList(shardId);
		...
		foreach(placementCell, placementList)
		{
			...
			if (connection != NULL &&
				ConnectionAccessedDifferentPlacement(connection, placement))
			{
				return true;
			}
		}
...
/*
 * ExtractSelectFromInsertSelect extracts the SELECT part from an INSERT...SELECT query.
