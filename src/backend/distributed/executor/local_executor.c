...
uint64 InLocalQueryOnShard = INVALID_SHARD_ID;
...
LocallyExecuteUtilityTask(Task *task)
{
	 ...
	 */
	uint64 prevInLocalQueryOnShard = InLocalQueryOnShard;

	if (task->anchorShardId != INVALID_SHARD_ID)
	{
		InLocalQueryOnShard = task->anchorShardId;
	}
	...
	InLocalQueryOnShard = prevInLocalQueryOnShard;
	if (InLocalQueryOnShard > 0 ) elog(WARNING, "InLocalQueryOnShard: %lu", InLocalQueryOnShard);
