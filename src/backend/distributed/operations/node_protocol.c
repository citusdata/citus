...
CreateSecurityLabelCommands(Oid relationId)
{
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		if (subObjectId > 0)
		{
			...
			names = lappend(names, makeString(relationName));
			...
			const char *attributeName = NameStr(attrForm->attname);
			names = lappend(names, makeString(quote_identifier(attributeName)));

			secLabelStmt->object = (Node *) names;
		}
		else
		{
