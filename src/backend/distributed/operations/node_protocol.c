...
CreateSecurityLabelCommands(Oid relationId)
{
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		if (subObjectId > 0)
		{
			...
			appendStringInfo(&buffer, "%s.", relationName);
			...
			Form_pg_attribute attributeForm = TupleDescAttr(tupleDescriptor, subObjectId -
															1);
			const char *attributeName = NameStr(attributeForm->attname);
			appendStringInfo(&buffer, "%s", quote_identifier(attributeName));

			secLabelStmt->object = (Node *) makeString(buffer.data);
		}
		else
		{
			Assert(subObjectId == 0);
			secLabelStmt->objtype = OBJECT_TABLE;
			secLabelStmt->object = (Node *) makeString(pstrdup(relationName));
		}
