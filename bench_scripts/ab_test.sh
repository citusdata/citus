...
echo "=== FAST PATH OFF ==="
psql -p $PORT -d $DB -tA -c "ALTER SYSTEM SET citus.enable_single_task_execution = off;"
...
echo ""
echo "=== FAST PATH ON ==="
psql -p $PORT -d $DB -tA -c "ALTER SYSTEM SET citus.enable_single_task_execution = on;"
psql -p $PORT -d $DB -tA -c "SELECT pg_reload_conf();"
sleep 1
for i in $(seq 1 $ITERS); do
    pgbench -p $PORT -d $DB -f bench_scripts/select.sql -c 1 -j 1 -T $DURATION --no-vacuum 2>&1 | grep "tps ="
