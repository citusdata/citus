...
* restrict SSL Ciphers to TLS1.2 and above

* Turn on ssl by default for new installations of citus

* Do not allow GROUPING SET clauses in subqueries
...
* Add validate constraint sql query support

* Improved security in task-tracker protocol

* Add user ID suffix to intermediate files generated by task-tracker

* Execute SQL tasks using worker_execute_sql_task UDF when using task-tracker

* Fixes permission checks in mark_tables_colocated, upgrade_to_reference_table, lock_relation_if_exists

* Improves permission checks in internal DROP TABLE functions

* Support round-robin `task_assignment_policy` for queries to reference tables.

* Improves permission checks in cluster management functions

* Refactor code that handles DDL commands from one file into a module
